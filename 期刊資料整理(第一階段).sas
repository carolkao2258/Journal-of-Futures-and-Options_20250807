...
proc sort data=avg_PQ_final2_sorted2 nodupkey; by companyID year; run;
data avg_PQ_lag;
...
/* 先排序 year companyID */
proc sort data=patent_regression_data1_num out=patent_regression_data1_srt;
  by year companyID;
run;

/* 調整欄位順序：指定欄位放最前，其他自動接續 */
proc sql noprint;
    /* 取得除了指定欄位以外的其他欄位清單 */
    select name into :other_cols separated by ' '
    from dictionary.columns
    where libname='WORK' and memname='PATENT_REGRESSION_DATA1_SRT'
...
/* 重新建立資料集，欄位順序：指定欄位 + 其他欄位 */
data patent_regression_data1_srt;
    retain year companyID companyName 
           invention_acc new_acc design_acc total_acc
           invention_sin new_sin design_sin ROE &other_cols;
    set patent_regression_data1_srt;
run;
