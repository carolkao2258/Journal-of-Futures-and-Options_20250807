...
/* ========= 路徑 ========= */
%let main_xlsx = /home/u64061874/patent_regression_data1_announce.xlsx;   /* 來源檔（原值保留） */
...
data patent_data_num;
  ...
  /* 統一清洗：含逗號、NBSP、'-' 等；尾碼為 '%' 就 /100 轉小數 */
  %macro mknum(src, dst);
    length &dst 8.;
    if vtype(&src)='N' then &dst=&src;
    else do;
      _s = strip(&src);
      _s = tranwrd(_s,'，',',');                    /* 全形逗號 -> 半形 */
      _s = compress(_s, ', '||byte(160));           /* 去千分位逗號、空白、NBSP */
  ...
  %mknum(GDP_growth,          nGDP);           /* '6.85%' -> 0.0685；若原本是 0.0422 也可直接讀 */
...
/* ========= 3) 每年 brokerage_income 總和（market_share 分母，用數值版） ========= */
...
/* ========= 8) 匯入 marginFac (marginFac1, marginFac2) ========= */
...
/* ========= 9) 合併 → data3（不刪 observation） ========= */
/* 用 CATS() 讓 companyID、year 兩邊自動轉成字元再比對，避免型別不一致 */
proc sql;
...
proc export data=patent_regression_data3
  outfile="&out3_xlsx"
  dbms=xlsx replace;
  sheet="patent_regression_data3_announce";
  putnames=yes;
run;
...
proc export data=patent_regression_data4
  outfile="&out4_xlsx"
  dbms=xlsx replace;
  sheet="patent_regression_data4_announce";
  putnames=yes;
run;
