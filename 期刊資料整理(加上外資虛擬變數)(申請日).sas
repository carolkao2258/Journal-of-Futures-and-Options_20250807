...
filename p3_1 "/home/u64061874/patent_regression_data3_apply.xlsx";
filename out5 "/home/u64061874/patent_regression_data5_apply.xlsx";

/* ========== 1) 匯入資料 ========== */
proc import datafile=fc   out=foreign_company_raw           dbms=xlsx replace; getnames=yes; run;
proc import datafile=p3_1 out=patent_regression_data3_apply dbms=xlsx replace; getnames=yes; run;
...
%normalize_companyid(in=patent_regression_data3_apply,
                     out=p3_norm,
                     src_id=companyID);
...
/* ========== 3) 合併（左併，以 p3_norm 為主；companyID 文字比對） ========== */
proc sql;
    create table p5_join as
    select 
        a.*,
        b.foreign_company,
        b.foreign_dummy
...
proc sort data=p5_fil out=p5_sort; by year companyID; run;

data p5_final; /* 這就是要匯出的工作資料集（短名） */
    retain 
...
proc export data=p5_final
    outfile=out5 dbms=xlsx replace;
    sheet="patent_regression_data5_apply";
    putnames=yes;
run;
